...
var prefixSumWithColor: [Int] = Array(repeating: 0, count: N + 1)   // prefixSumWithColor[i]: 현재까지 발견된 공들중 i색을 가진 공들의 크기 누적합
...
for i in 0..<N {
    ...
    if i > 0 && arr[i].size == arr[i - 1].size && arr[i].color == arr[i - 1].color {
        // 현재 탐색중인 공의 색깔, 크기가 이전과 동일하면 이전의 결과를 가져다가 쓴다. 여기가 문제해결의 핵심..
        ...
        currSum = totalSum - prefixSumWithColor[arr[i].color] - (numberOfBallWithSameSize[arr[i].size] * arr[i].size)
    }
    result.append((arr[i].index, currSum))
    
    totalSum += arr[i].size
    prefixSumWithColor[arr[i].color] += arr[i].size
