...
while true {
    ...
    arr = line.split(separator: " ").map { Int(String($0))! }
    N = arr.removeFirst()
    
    segmentTree = .init(array: arr, function: { a, b in arr[a] < arr[b] ? a : b })  // 높이가 더 작은 배열의 인덱스를 저장
...
var segmentTree: SegmentTree<Int>
*/
